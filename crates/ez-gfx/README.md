...
A globally synchronized generational arena allocates unique context handles. Each non-`Send` context state lives in creator-thread-local storage, and every context/resource operation remains creator-thread-affine. Independent creator threads do not share a state mutex. Creator-thread exit invalidates remaining handles and performs best-effort terminal cleanup before native state drops; call `destroy_context` explicitly to observe cleanup failures.
