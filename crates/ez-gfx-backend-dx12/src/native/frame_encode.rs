...
impl DxFrameEncoder<'_> {
    fn finish_pass(&mut self) -> Result<(), HalError> {
        if !self.pass_active {
    ...
    fn capture_present(&mut self, capture_presented: bool) -> Result<(), HalError> {
        if self.pass_active {
            return Err(HalError::InvalidArgument);
        }
        if capture_presented {
            ...
            unsafe {
                ...
                copy_texture_to_readback(self.list, back_buffer, &readback.resource, *footprint);
                record_resource_barriers(
                    self.list,
                    [transition_barrier(
                        back_buffer.clone(),
        ...
        actions.visit(&mut |action_index, action| {
            match action {
                ...
                NativeFrameAction::CopyTexture {
                    ...
                    destination,
                    region,
                } => self.copy_texture(source, destination, *region)?,
                ...
                NativeFrameAction::Present => {
                    self.capture_present(capture_presented)?;
                }
            }
            Ok(())
        })?;
