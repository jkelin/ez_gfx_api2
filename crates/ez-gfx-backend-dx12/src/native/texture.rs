...
impl NativeContext {
    ...
    /// RGBA16F plus depth attachment support for D32 float. Multisample
    /// ceilings come from per-format quality-level queries.
    ///
    /// # Errors
    ///
    ...
    pub fn probe_target_formats(
        ...
        let query = |format, dxgi| {
            ...
            .map_err(|error| map_allocation_windows(&error))?;
            // Quality queries never fail the probe: a rejected count simply
            // leaves a lower ceiling.
            let ceiling = max_sample_count(&self.device, dxgi);
            Ok::<_, AllocationError>(support_for_target_format(format, data.Support1, ceiling))
        };
        let supports = [
