...
The Win32 [`C textured cube`](examples/c/textured_cube/README.md) uses ABI v18 directly, compiles its Slang source during the CMake build, and supports Vulkan or DX12 on a capable local host. Its CI job only builds and links the C11 program; it does not execute it. Hosted Windows DX12 remains compile-only because the runner does not guarantee a feature-level 12.1 adapter.
