...
## Build and examples
Install [mise](https://mise.jdx.dev/) and run `mise tasks` for build, test, example, and packaging commands. Run `mise run test`: mise resolves pinned cargo-nextest 0.9.101, nextest runs all workspace tests without fail-fast with each test bounded at 120 seconds, then workspace doctests run; retries are not configured. Each non-distributed Rust example names its adjacent Slang source and compiles validated artifact bytes in memory at process startup; no `.ezgfxshader` is tracked. These development binaries intentionally carry compiler tooling. Runtime crates and packaged runtime distributions remain compiler-free. `mise run examples-smoke` exercises the Rust scenes. See [`examples/README.md`](examples/README.md).

The Win32 [`C textured cube`](examples/c/textured_cube/README.md) uses ABI v18 directly, compiles its Slang source during the CMake build, and supports Vulkan or DX12 on a capable local host. Its CI job builds and links the C11 program, then smokes Vulkan with SwiftShader. Hosted Windows DX12 remains compile-only because the runner does not guarantee a feature-level 12.1 adapter.
