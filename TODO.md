...
- Complete Metal execution evidence for deterministic adapter enumeration, selection, admitted limits/formats, automatic texture fallback-to-real publication, and rejection diagnostics through safe Rust and ABI 41 (P-003, P-022).
...
- Add optional minimum resident/initially-loaded mip level to `TextureConfig`: prioritize those required mips first per texture, gate frame submission on the GPU timeline for required-mip readiness instead of CPU/global `wait_idle`, then remove the Example snapshot `wait_idle`; land as one synchronized Vulkan/DX12/Metal contract with safe-Rust ownership and FFI/ABI validation.
