...
/** Acquires a context-owned counted buffer with an explicit element stride. */
EzGfxResult ez_gfx_counted_buffer_acquire(uint32_t element_size, uint32_t element_count, const char *debug_name, size_t debug_name_length, EzGfxCountedBuffer *out_buffer, EzGfxContext context) EZ_GFX_ACCESS(read_only, 3, 4) EZ_GFX_ACCESS(write_only, 5);
/** Writes a contiguous draw-command range into a counted buffer. */
EzGfxResult ez_gfx_counted_buffer_write_draws(EzGfxCountedBuffer buffer, uint32_t start_index, const EzGfxDrawIndexedCommand *commands, uint32_t command_count, EzGfxContext context) EZ_GFX_ACCESS(read_only, 3, 4);
/** Publishes a CPU-known count for commands generated by a compute node. */
EzGfxResult ez_gfx_counted_buffer_publish_count(EzGfxCountedBuffer buffer, uint32_t count, EzGfxContext context);
/** Releases a context-owned counted buffer; zero and stale values are ignored. */
void ez_gfx_counted_buffer_release(EzGfxCountedBuffer buffer, EzGfxContext context);
...
/** Acquires a context-owned buffer before frame recording. */
...
/** Copies a validated element range into a context-owned buffer. */
...
/** Releases a context-owned buffer; zero and stale values are ignored. */
