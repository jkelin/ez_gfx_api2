...
mod renderer {
    ...
    use ez_gfx::{
        ...
        ShaderRequest, Stage, acquire_indirect, acquire_structured, create_index_heap,
        destroy_index_heap, destroy_shader, load_shader, release_indirect, release_structured,
        render_add_graphics, set_indirect_count, upload_indices, write_indirect, write_structured,
    };
        ...
        pub(super) fn create(context: u64) -> Result<Self, String> {
            ...
            let first_index = match upload_indices(
                ...
                Err(error) => {
                    ...
                    return Err(format!("upload indices: {error:?}"));
                }
            };
            ...
            let indirect = match acquire_indirect(context, 1) {
                ...
                Err(error) => {
                    release_structured(context, positions_handle);
                    destroy_index_heap(context);
                    return Err(format!("acquire triangle indirect buffer: {error:?}"));
                }
            };
            ...
            let shader = match load_shader(context, include_bytes!("01_triangle.ezgfx"), &requests)
            {
                Ok(handle) => handle.get(),
                Err(error) => {
                    release_indirect(context, indirect);
                    release_structured(context, positions_handle);
                    destroy_index_heap(context);
                    return Err(format!("load triangle artifact: {error:?}"));
                }
            };
