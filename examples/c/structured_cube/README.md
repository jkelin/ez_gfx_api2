...
cmake -S examples/c/structured_cube -B target/c-examples/structured_cube-build
cmake --build target/c-examples/structured_cube-build --config Debug
```
...
$Build = Join-Path $PWD "target\c-examples\structured-cube-package"
cmake -S examples/c/structured_cube -B $Build `
  ...
  -DEZ_GFX_SHADER_ARTIFACT="$Build\structured_cube.ezgfxshader"
cmake --build $Build --config Release
```

CMake compiles the JSON manifest; no artifact is tracked.

Run one bounded frame and require a nonempty raw 640×480 RGBA snapshot:

```powershell
target/c-examples/structured_cube-build/Debug/structured_cube.exe --backend vulkan --artifact target/c-examples/structured_cube-build/Debug/structured_cube.ezgfxshader --max-frames 1 --snapshot target/c-examples/vulkan.rgba
target/c-examples/structured_cube-build/Debug/structured_cube.exe --backend dx12 --artifact target/c-examples/structured_cube-build/Debug/structured_cube.ezgfxshader --max-frames 1 --snapshot target/c-examples/dx12.rgba
```
