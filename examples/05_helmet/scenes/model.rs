...
impl ModelScene {
    pub fn create(context: u64) -> Result<Self, String> {
        let bytes = include_bytes!("../helmet.glb").as_slice();
        let camera = OrbitCamera::new(35.0_f32.to_radians(), 22.0_f32.to_radians(), 5.0);
        let target = [0.0, 0.0, 0.0];
        let near = 0.1;
        let far = 100.0;
        let artifact = include_bytes!("../05_helmet.ezgfx").as_slice();
        ...
        let records = basic_records(&mesh, first_index);
        let primitive_bytes = records.len() * std::mem::size_of::<PrimitiveBasic>();
...
fn basic_records(mesh: &MeshData, first_index: u32) -> Vec<PrimitiveBasic> {
    mesh.primitives
        .iter()
        .map(|primitive| PrimitiveBasic {
