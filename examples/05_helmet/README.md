...
`main.rs` reads top-to-bottom: setup loads `helmet.glb` and creates owning geometry and shader wrappers. The linear frame loop acquires one-frame structured and counter buffers, explicitly configures the swapchain, records compute then graphics, and passes the frame and target to `Example::handle_frame`.
