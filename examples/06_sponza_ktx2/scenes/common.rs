...
use ez_gfx_ffi::{
    EzGfxBinding, EzGfxDynamicState, EzGfxResult, EzGfxShaderEntry, EzGfxTextureDesc,
    ez_gfx_acquire_indirect, ez_gfx_index_heap_create, ez_gfx_index_heap_destroy,
    ez_gfx_indirect_release, ez_gfx_indirect_set_draw_count, ez_gfx_render_add_compute_pipeline,
        ...
        status(
            {
                ...
                unsafe { ez_gfx_acquire_indirect(capacity, name.as_ptr(), &mut handle, context) }
            },
        ...
        status(
            {
                ...
                unsafe {
                    ez_gfx_texture_load(bytes.as_ptr(), bytes.len(), &desc, &mut handle, context)
                }
            },
            "load texture",
        )?;
        ...
        if let Err(error) = status(
            {
                ...
                unsafe { ez_gfx_texture_get_binding(handle, &mut binding, context) }
            },
...
pub fn bytes_of<T>(value: &T) -> &[u8] {
    ...
    unsafe {
        core::slice::from_raw_parts(core::ptr::from_ref(value).cast(), std::mem::size_of::<T>())
    }
}
