...
impl ModelScene {
    pub fn create(context: u64) -> Result<Self, String> {
        ...
        let camera = OrbitCamera::new((-30.0_f32).to_radians(), 52.0_f32.to_radians(), 2.2);
        let target = [0.0, 0.55, 0.0];
        let near = 0.1;
        let far = 500.0;
        let artifact = include_bytes!("../03_compute_structured.ezgfx").as_slice();
