...
use crate::scenes::{FrameInput, SceneInput, SceneKey, SceneState, TexturedCube};
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        .and_then(|()| {
            TexturedCube::create(self.context).map(|resources| self.resources = Some(resources))
        });
