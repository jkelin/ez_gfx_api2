...
**Implementation status:** Complete in artifact format v6. Each metallib records Apple platform, CPU architecture, minimum OS, build SDK, Metal language version, metallib contract version, and Apple toolchain identity. The compiler obtains SDK/toolchain inputs from `xcrun`, derives architecture and deployment minimum from the build environment, and emits explicit Vulkan SPIR-V and DX12 Shader Model contracts alongside Metal metadata. Runtime filters incompatible metallibs before reflection or native library creation, and rejects development MSL.
Task/mesh Metal products fail closed below deployment minimum macOS 13, whether from an explicit deployment target or host provenance.
Mesh availability requires macOS 13+ plus a qualifying family (Apple7/8/9, Mac2); family identifiers alone prove nothing pre-Metal-3. Mesh grid ceilings are 1,024 threadgroups without reported Apple9 support and 1,048,575 with it. Pipeline-creation failures naming `maxTotalThreadsPerMeshThreadgroup`/`maxTotalThreadsPerObjectThreadgroup` in a library NSError (codes 1/3) classify as `Unsupported`; other native failures stay `NativeFailure`.
Measured: mesh-only and task+mesh fixtures emitted real AIR/metallib via `xcrun` under the same toolchain.
