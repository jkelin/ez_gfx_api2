...
The first fatal result atomically transitions `ContextInner` to `Lost`. New work fails; unsignaled tokens, leases, queued transfers, and callbacks complete exactly once with typed loss/cancellation; no teardown path waits for GPU progress. Existing owning wrappers retain memory-safe context ownership but their GPU identities remain permanently invalid. Bounded diagnostics survive until the last owner drops. The host creates a new `Context`, `Surface`, and resource wrappers.

Reject managed recreation because it requires retained asset bytes or reload callbacks, cross-module replay semantics, and transparent resource replacement not requested by the migration.
...
Assumptions: applications can rebuild state after receiving loss. Risks are double completion, callback races, reference cycles, deadlock during shutdown, missing diagnostics, and mistaken completion of GPU-owned memory. Inject loss at begin, submit, present, transfer, and idle paths; assert bounded completion and zero GPU waits; exercise wrapper drop orders and stale Rust/FFI identities.
