...
`EasyGraphicsCompiler::compile_shader` compiles every entry point discovered in one Slang module, including multiple names in the same stage, for each requested SPIR-V, DXIL, and Metal target and returns one validated `CompiledShader`. Slang `Amplification` maps to artifact `Task` and Slang `Mesh` to artifact `Mesh`; SPIR-V task/mesh discovery and compilation use the `spvMeshShadingEXT` capability, and task/mesh Metal products require deployment minimum macOS 13. Extract target declarations and common interface metadata before optimization can erase intent, then pass target blobs and canonical `(stage, entry-point name)` reflection to P-006. Use target-specific binding validation rather than a second semantic compiler pipeline.
...
**Validation:** compile a fixture with two same-stage entry points for every requested target; save and reload it; prove both exact names survive independently. A same-name vertex/fragment fixture proves exact `(stage, name)` resolution, and per-target products assert full target coverage for every expected stage/name pair. Compile every shader/entry point for all targets in CI runners with required native tools; compare canonical reflection and explicit target attributes across outputs; create pipelines and render snapshot fixtures on each backend; record compiler version/options, wall time, peak RSS, and blob sizes. Completion remains contingent on the recorded backend matrix. Measured: macOS `shader_compile` 13/13 green (Slang 2025.23.2, Xcode 26.3 build 17C529, macOS SDK 26.2, rustc/cargo 1.95.0); Windows `shader_compile` 12/12 green with DXIL via VulkanSDK 1.4.335.0 (dxcompiler.dll 1.10 / 1.8.0.5123, dxil 1.7).
