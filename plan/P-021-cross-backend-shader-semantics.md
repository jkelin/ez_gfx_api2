...
Runtime owns compiler-free `.ezgfxshader` loading. A host-side `CompiledShader` contains all discovered stage/name variants; loading one exact stage/name returns an owning, context-bound generational shader identity. Compute and graphics pipelines are created on demand, keyed by every shader identity and relevant state, and evicted on shader drop into backend completion-tracked deferred retirement. Mesh pipelines take exact task/mesh/fragment products with reflected per-threadgroup sizes validated against native grid/threadgroup limits; bindings stay stage-visible to task/mesh/fragment only. Warmed mesh cache-key preparation across alternating task/taskless keys and texture snapshots reuse caller scratch with no steady-state allocation.
