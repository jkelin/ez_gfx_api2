...
Buffer and counter resources bind by shader-declared name: `[Buffer("name")]` and `[CounterBuffer("name")]` reflect as `buffer` and `counter_buffer` binding kinds. `Frame::bind_buffer` adds or replaces one named entry in the frame binding set without materializing a never-executed replacement. `execute_compute`, `execute_graphics`, and `execute_mesh` materialize and read the current set without removing entries, so unchanged resources remain bound across same-frame work. Mesh execution requires mesh and fragment stages, accepts an optional task stage, and dispatches the supplied nonzero group grid through task when present or mesh otherwise; per-threadgroup dimensions always come from reflection. Single POD constants bind the same way as `ValueBuffer`, not as push-constant bytes. ABI 40 exposes the same persistent frame-local set through `ez_gfx_frame_bind` and `ez_gfx_frame_execute_compute`/`graphics`; terminal end or abort clears it.

ABI 41 adds `EzGfxTaskShader`, `EzGfxMeshShader`, stage-specific load/destroy exports, `EzGfxMeshShaders`, and `ez_gfx_frame_execute_mesh(context, frame, ...)`. The shader descriptor is required and aligned; mesh and fragment handles are required, while a zero task handle means absent. Optional `EzGfxMeshState` defaults to no culling, counter-clockwise front faces, and no blending, and its reserved byte must be zero. `ez_gfx_context_shader_capabilities` reports task/mesh availability without changing caller output on failure; adapter enumeration includes the same stage bits.
