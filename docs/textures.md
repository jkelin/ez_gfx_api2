...
Device loss is terminal: the first observed `DeviceLost` marks the context and synchronously cancels every queued decode, so later polls return `DeviceLost` immediately instead of `NotReady`. A loss-poisoned transfer worker keeps the loss reason, so later admissions report `DeviceLost` rather than a generic `NativeFailure`.

Context destruction stays creator-thread-affine. Callers under Windows loader lock must use the abandon path only (thread-exit invalidation without native cleanup or joins), never `destroy_context`/`wait_idle`.
...
Each backend also probes render-target roles from the device (`probe_target_formats`): Vulkan reads optimal-tiling features, Direct3D 12 queries format support, and Metal reports its architecture-guaranteed table. Depth candidates resolve only with attachment support; otherwise resolution fails with `UnsupportedFormat` instead of selecting an unusable format.
...
| --- | --- | --- |
| Vulkan | Dedicated transfer family where available; otherwise secondary graphics-family queue or universal queue. Texture and geometry use separate queues when possible. | Timeline semaphores order graphics release, copy, and acquire. The owner joins transfer completion before enqueueing the graphics acquire: submitting it early would pend on the shared graphics queue ahead of later frames, breaking coarse sampling while fine work is blocked. Four retiring command slots bound reuse; a slot recycles only after its completion signals. |
| Direct3D 12 | Independent texture/geometry COPY workers; DIRECT queue performs graphics transitions. | The owner waits for copy completion before enqueueing the DIRECT transition (COPY lists reject shader-resource transitions, so the transition cannot ride the copy list; submitting it early on the shared DIRECT queue makes later samples inherit the gated wait). Coarse frames need not wait for later fine copies. Four allocator/list/event slots retire by actual completion; a busy slot is the only other blocking case. |
| Metal | Independent geometry/texture queues, shared texture blit command buffers, independent BC/ASTC queries. | GPU events order prior reads and later sampling. Texture owners commit without waiting for execution and observe failure through status polling; an earlier errored buffer fails the next submission without blocking. Cancellation-only batches commit a real signal. Geometry workers track ordered admission, and graphics waits only for the referenced accepted buffer command. |
...
Before graphics waits, all backends flush each referenced transfer owner only through the required accepted token. An already-submitted token returns without draining later stages. Metal submit callbacks return after driver submission with GPU completion observed through completion values; Vulkan and Direct3D 12 callbacks additionally join GPU completion so transfer-completion work never queues ahead of later frames on the shared graphics queue. Shutdown, wait-idle, and destroy-context drains still join actual GPU work terminally, as does Metal's referenced buffer-command wait on the calling thread before graphics submission. Ring exhaustion (all slots busy) remains bounded backpressure by design. A dedicated transition/acquire queue per worker would remove the remaining host waits; that device-init change is future work. Measured wall costs appear below.
...
| `create_render_target` | `ez_gfx_render_target_create` |
| `destroy_render_target` | `ez_gfx_render_target_destroy` |
| `render_target_format` | `ez_gfx_render_target_get_format` |
| `render_target_extent` | `ez_gfx_render_target_get_extent` |
| `render_target_clear` | `ez_gfx_render_target_get_clear` |
| `probe_render_target_format` | `ez_gfx_render_target_probe_format` |
| `begin_render_target` | `ez_gfx_begin_render_target` |
...
ABI **25** adds managed render-target exposure without changing existing layouts: `EzGfxRenderTargetFormat` codes mirror the runtime format discriminants (`1..=6`), `EzGfxRenderTargetUsage` names color/depth/storage/sampled access (`0..=3`), and `EzGfxRenderTargetDesc` borrows its name (required 1..=255 UTF-8 bytes) and candidate codes (1..=16 entries) only for the creating call. Depth, storage, and sampled-only declarations stay `Unsupported` with explicit errors; multisampled color declarations (probed ceiling per backend) allocate render storage that resolves into the sampled image at end of pass. Multisampled draws stay `Unsupported` until pipelines carry sample counts. Render targets lease heap slots from the shared texture registry (one allocator owns both, slot index is the descriptor binding), so texture and target bindings never collide and share the 1024-slot heap cap. C pointer ranges are copied before delegation and output pointers are written only on success; no panic or Rust value crosses the boundary.
...
- Linux `texture_pixels` accepts only `Unsupported` from the first presentation-dependent frame when the selected headless ICD lacks WSI; `NativeFailure`, unknown statuses, and later failures remain test failures. Native render-target allocation, clear/readback, and MSAA proof remains separate. Lavapipe's headless surface captures UNORM attachment bytes before its `SRGB_NONLINEAR` presentation transform; Windows and Metal capture sRGB attachments. Thus BC7 mode-6 linear low channels remain 1 in Lavapipe capture but encode from 1/255 to 13 in sRGB attachment capture; sRGB BC7 value 1 decodes below one Lavapipe UNORM output step.
- Render-target slice (this change): native single-mip sampled creation/probing tests pass on RTX 3080 Vulkan (2/2) and DX12 (2/2) plus Apple M2 Pro Metal probe/allocation (2/2 reported remotely, not re-verified in this tree; clear→readback unrun — remote Mac run pending); safe `create/query/destroy` validation (empty extents, depth-usage deferral, unknown handles) passes headlessly, while live-target creation needs an initialized device with a real surface and stays proven at the native level. Full suites re-verified: `ez-gfx` lib 24/24, example smoke 32/32, shader artifacts 5/5, host BC pixel suites 2/2 (Vulkan+DX12, including ASTC `Unsupported` on both RTX backends). Eviction re-confirmed as descriptor/view narrowing only: all three backends replace the sampled view and retain the full image allocation; memory retires on unload, never on demotion.
- Render-target pass attachment + binding hygiene (this change): native `BeginPass` carries one `PassAttachment` per color (surfaces keep `SURFACE_DEFAULT_CLEAR`, targets carry their stored declaration clear) with per-attachment extent checks and depth+RT / `samples != 1` rejection on Vulkan, DX12, and Metal (Metal Mac run pending); `destroy_render_target` (plus context teardown) and `frame_begin` now clear the bound override, pinned by two headless safe tests. Evidence on RTX 3080: Vulkan lib 23/23 and DX12 lib 13/13 green serially, safe RT-focused 4/4 green.
- Heap-slot unification (this change): render targets lease `TextureRegistry` slots (`begin_upload`/`reserved_binding`/`cancel_upload`) instead of the interim top-down range, so one allocator owns textures and targets with no binding collisions and a shared 1024-slot cap; rejected admissions still fail before leasing and destroy/teardown release via `cancel_upload` without texture unload events. Evidence on RTX 3080, all serial with fresh target dirs: Vulkan lib 23/23, DX12 lib 13/13, `ez-gfx` lib 32/32 (incl. four new headless allocator tests: collision, release-reuse, shared exhaustion, rejection residue), FFI `abi` 25/25 (v25 parity intact, ABI unchanged). Metal is Apple-gated code-review only, Mac run pending.
- MSAA render-target resolve (this change): multisampled color targets allocate render storage beside the sampled image and resolve at end of pass — Vulkan via resolve attachments (`AVERAGE` into the sampled view), Direct3D 12 via `ResolveSubresource` at `EndPass`, Metal via `resolveTexture` + `MultisampleResolve` (eye-reviewed only, Mac run pending). Probes report real ceilings (Vulkan image-format queries, D3D12 quality levels, Metal device-wide sample-count query); draws into multisampled targets stay `Unsupported` until pipelines carry sample counts. Evidence on RTX 3080, serial: clear→resolve→readback exact-byte tests green on Vulkan and DX12, plus ceiling-gated FFI lifecycle coverage; ABI stays 25.
- Linux RTX 3090 validation exposed a generic barrier incorrectly moving color-only Vulkan MSAA storage to `SHADER_READ_ONLY_OPTIMAL`. The sampled resolve image still follows graph layouts; MSAA storage now enters and remains in `COLOR_ATTACHMENT_OPTIMAL`. The exact resolve test and full 28-test backend suite are validation-clean.
- Parallel-stall pin (RTX 3080 Windows, driver wedge, tracked in root TODO.md): the 23/23 and 13/13 counts above are serial-only evidence, not full proof — both suites are pinned serial in-harness (`.config/nextest.toml` `gpu-serial` group for `cargo nextest`, `.cargo/config.toml` `RUST_TEST_THREADS = "1"` for bare `cargo test`), so default invocations now run serially with no thread flags. Unpinned default parallelism stalls (Vulkan: 9 GPU tests, 0 GPU completions in >12min; in-session `--test-threads=8`: 13 CPU tests pass, all GPU-device tests stuck >60s; `--test-threads=4` green in 1.38s). Per-test gates/observers/temp files ruled out; suspected concurrent per-test device creation at >=8 concurrent creations.
- Render-target C ABI (this change, ABI 25, RTX 3080 Windows): seven new exports (`create/destroy/get_format/get_extent/get_clear/probe_format/begin_render_target`) delegate to the safe layer with null/count/enum/UTF-8/out-pointer validation and panic containment; depth/storage/sampled creation and depth/above-ceiling probing report `Unsupported` on Vulkan and DX12 hidden contexts, while ceiling-admitted multisample creation passes the full lifecycle. Evidence, all serial with fresh target dirs: Vulkan lib 23/23, DX12 lib 13/13, `ez-gfx` lib 28/28 (incl. the new headless probe test), FFI `abi` 25/25 (incl. layout/export-parity plus hidden Vulkan+DX12 lifecycle tests), `xtask` declaration parity green, `export-parity` 57 functions against the built dll with zero forbidden imports, header compiles as C11/C++17. Metal adds no backend code (the safe probe dispatches through the existing per-backend constructors); Mac run pending.
...
Batch counts above are unaffected by per-batch wait removal (bundling is unchanged). No Metal wall times are recorded here; Windows timings must not be compared across the change.
...
- [P-012](../plan/P-012-transfer-queue-staging-and-batching.md#selected-solution): pooled staging, independent streams, and native cross-texture batching are implemented and GPU-tested. Metal texture owners commit without waiting for execution (overlap regression submits a second batch while the first is GPU-pending). Vulkan and Direct3D 12 keep targeted host waits as explicit exceptions: early transfer-completion submits pend on the shared graphics queue ahead of later frames (proven by coarse-while-blocked regressions), and COPY lists reject shader-resource transitions. Ring exhaustion and shutdown/wait-idle drains still block by design; the frame-slot reap only observes completion. A dedicated transition/acquire queue per worker is future work.
...
Remaining evidence includes broader transcode/multicore/full-scene performance and Metal atlas/staging benchmarks; Windows timings are not extrapolated to Apple. A remote Mac run re-verifying the Metal RT probe/allocation claim and running the clear→readback test is still pending. Native evidence is specific to the tested adapters and scenarios, not every Metal device or full-plan completion. See [root TODO](../TODO.md).
